...
import { ICON_CDN_URL_18, LICENSE_CDN_URL } from "../Helpers/Constant";
import {
  faIndianRupeeSign,
  faCircleHalfStroke,
  faStar,
...
const RestaurantMenu = () => {
  const { id } = useParams();
  const [resDetails, resOffers, resMenu, resLicense, resAddress] =
    ...
    <MyContext.Provider value={contextValue}>
      <div className="min-h-screen mx-auto mt-32 max-w-[75%]">
        ...
        <div className="flex justify-between my-5">
          <div>
            <h1 className="font-black text-xl text-[#282c3f] pb-2">
              {resDetails.name}
            </h1>
          ...
          {resDetails.avgRatingString && (
            <div className="flex flex-col p-2 h-fit border border-gray-300 shadow-sm rounded-md">
              ...
              <h4 className="text-[10px] text-[#8b8d97] font-bold tracking-tighter text-shadow-xs">
        ...
        {resDetails.feeDetails.message ? (
          <div className="flex pb-3">
            <h4 className="flex items-center justify-center text-base text-[#7e808c] font-light ">
              <img
                src={ICON_CDN_URL_18 + resDetails.feeDetails.icon}
                className="pr-2"
              />{" "}
              {resDetails.feeDetails.message}
            </h4>
            ...
            {resDetails.costForTwoMessage ? (
              <h2 className="text-base text-[#282c3f] font-extrabold text-shadow-md">
                <FontAwesomeIcon
                  ...
                  className="px-[5px] py-[1.5px] text-sm rounded-full border-[1px] border-solid border-[#282c3f] text-[#282c3f]"
                />{" "}
                &nbsp;
                {resDetails.costForTwoMessage}
              </h2>
        ...
        <div>
          {resLicense.map((x) => {
            return (
              <div
                key={x}
                className="flex items-center text-[#93959f] bg-[#f1f1f6] mt-4 py-4 pl-4">
                <img
                  src={LICENSE_CDN_URL + x.imageId}
          ...
          {resAddress.map((x) => {
            return (
              <div className="text-[#93959f] bg-[#f1f1f6] px-4 pb-32" key={x}>
                <h3 className="border-t border-gray-300 font-extrabold text-lg pt-4">
                  {x.name}
                </h3>
                <h4 className="text-sm font-normal tracking-tighter">
                  (Outlet : {x.area})
                </h4>
        ...
        {resMenu.length !== 0 && showElement && (
          <div
            className={`bottom-8 left-[50%] -translate-x-1/2 flex z-40 text-center pb-3 pt-3 px-4 text-white rounded-full bg-[#5d8ed5] w-fit whitespace-nowrap sticky shadow-[0px_8px_12px_0px_#0000004d]`}>
            <svg
            ...
            <button className="leading-3 pl-2 tracking-tight text-sm font-black self-end">
              BROWSE MENU
            </button>
          </div>
        )}
